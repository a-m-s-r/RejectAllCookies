...
export type ActionMeaning = 'rejectAll' | 'openPreferences' | 'disableAll' | 'objectAll' | 'save' | 'unsafe' | 'unknown';

export function classifyAction(text: string): ActionMeaning {
  const normalized = normalizeText(text);
  if (!normalized || /\b(?:do not|don t|nicht|ne pas|no)\s+(?:reject|refuse|ablehnen|refuser)\b/u.test(normalized)) return 'unknown';
  if (matchesConcept(normalized, 'unsafePositive')) return 'unsafe';
  if (matchesConcept(normalized, 'necessaryOnly') || matchesConcept(normalized, 'reject')) return 'rejectAll';
