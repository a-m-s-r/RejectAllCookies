...
test('rejects a dynamically inserted banner without accepting and persists on reload', async ({ page }) => {
...
test('popup exposes local controls and a truthful status', async ({ page, extensionId }) => {
  ...
  await expect(page.getByRole('status')).toContainText(/No consent result|Rejected|handling|failed/u);
});
